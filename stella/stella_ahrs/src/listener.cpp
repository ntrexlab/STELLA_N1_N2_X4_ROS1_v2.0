...
MwAhrsRosDriver::MwAhrsRosDriver() 
{
    ...
    timer = n.createTimer(ros::Duration(0.2),&MwAhrsRosDriver::serial_callback,this);
}
...
void MwAhrsRosDriver::serial_callback(const ros::TimerEvent& event)
{
  ...
  { 
    ...
    imu_data_raw_pub_ = n.advertise<sensor_msgs::Imu>("imu/data_raw", 10);
    imu_data_pub_ = n.advertise<sensor_msgs::Imu>("imu/data", 10);
    ...
    imu_data_raw_msg.header.frame_id = imu_data_msg.header.frame_id =
        imu_magnetic_msg.header.frame_id = imu.header.frame_id = "lmu_link";
